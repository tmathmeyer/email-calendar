...
public class MouseHoverListener implements MouseListener
{
	...
	/**
	 * The number of pixels the mouse event must be from the component border to
	 * be considered an exit/entrance
	 */
	...
	{
		// Check to make sure mouse is exiting through the outer bounds of the
		// component
		if (e.getPoint().getX() < BUFFER || e.getPoint().getX() > (hoverable.getSize().getWidth() - BUFFER)
		        || e.getPoint().getY() < BUFFER || e.getPoint().getY() > (hoverable.getSize().getHeight() - BUFFER))
		{
