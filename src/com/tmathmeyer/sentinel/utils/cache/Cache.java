...
public class Cache<K, V>
{
	...
	Map<K, AccessOrderedList<TimeOrderedList<V, K>>> cache = new HashMap<K, AccessOrderedList<TimeOrderedList<V, K>>>();
	...
	/**
	 * insert a key value pair into the cache set it as most recently accessed
	 * as well as most recently inserted
	 * 
	...
	/**
	 * removes an item from the AccessOrderedList, but not from the
	 * TimeOrderedList
	 * 
	 * 
	...
	public void remove(K key)
	{
		...
		if (fromMap != null)
		{ // move it to the front so that the whole is cleaned up, then delete
		  // it from the map
			fromMap.access(head);
			cache.remove(key);
		}
	...
	/**
	 * get a value by key from the cache set it as most recently accessed but
	 * not as most recently inserted
	 * 
	...
	/**
	 * Get an iterator over the elements in the order that they were last
	 * accessed or inserted NOTE: this iterator will start at the least recently
	 * used element and end at the most recently used element. iterating over
	 * the elements in this way will not update their order
	 * 
	 ...
	 * NOTE: this iterator will start at the least recently inserted element it
	 * would be advisable, though not required, to apply each element to the
	 * AccessOrderedList so that the tail events will simply drop off and get
	 * scooped up by the garbage collection
	 * 
	 * @param key the key from the KVP to start the iterator at
